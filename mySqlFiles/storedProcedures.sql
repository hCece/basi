...
delimiter //
create procedure inserisciPrenotazione(in pro int, partenza varchar(20), arrivo varchar(20), Nposti int, usernameCliente varchar(20), bli int, spo , lus boolean, ele boolean)
begin
    ...
    set checkVal= exists(
        ...
        where ((citta = partenza) and (posti >= Nposti) and  (blindato = bli) and (lusso= lus) and (sportivo = spo) and (elettrico = ele)));
    else
		set checkVal= exists(
...
INSERT INTO  TASSISTA(CF,username,nome,cognome,dataDiNascita,targaAuto,citta) VALUES ('123c','jury15', 'juri', 'horstmann', '2000-05-01','123qwe','Bologna');
...
insert into TAXIPRO(targa,marca,modello,posti,lusso,elettrico) values ('123qwe','benz','slk',2,true,true);
